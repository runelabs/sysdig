	...
	struct event_data_t *event_datap)
{
	...
	if (unlikely(atomic_inc_return(&ring->preempt_count) != 1)) {
		 ...
		 * even if the page fault is disabled, if the address is illegal
		 * and would generate a segfault (e.g. NULL), it seems like the kernel still
		 * calls the page_fault_kernel tracepoint, even if the page fault
		 * is actually not executed (otherwise we'd see tons of warnings
		 * due to the might_sleep() macro). Still not sure how to solve this,
